...
lora_merged_ckpt_path=/data/DeepSeek-Coder/experiments-magi-lora-merged/deepseek-ai/deepseek-coder-6.7b-base/ise-uiuc/Magicoder-OSS-Instruct-75K/$LANGUAGE
largest_checkpoint=$(ls -d ${lora_merged_ckpt_path}/checkpoint-* 2>/dev/null | grep -oP 'checkpoint-\K[0-9]+' | sort -nr | head -n 1)
...
finetune/finetune_deepseekcoder_lt_sft.py \
    ...
    --full_ckpt_path $lora_merged_ckpt_path/checkpoint-${largest_checkpoint} \
    --data_path $DATA_PATH \
